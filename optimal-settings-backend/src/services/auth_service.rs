...
use crate::{models::auth::{Keys, UserCredentials, Claims}, error::AuthError};
...
pub fn authorize(credentials: &UserCredentials) -> Result<String, AuthError> {
    ...
    if credentials.username() != ADMIN_CREDETIALS.username() || credentials.password() != ADMIN_CREDETIALS.password() {
        return Err(AuthError::WrongCredentials);
    }
    let claims = Claims::new(credentials.username().to_owned(), (OffsetDateTime::now_utc() + Duration::days(1)).unix_timestamp() as usize);
