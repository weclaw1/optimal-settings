...
use crate::{repositories::{ReportRepository, Repository}, models::{Report, ValidateModel}};
...
pub async fn get_report(pool: &SqlitePool, report_id: u64) -> Result<Option<Report>, anyhow::Error> {
    let report_repository = ReportRepository::new(pool.clone());
...
pub async fn get_reports_by_game(pool: &SqlitePool, game_id: u64) -> Result<Vec<Report>, anyhow::Error> {
    let report_repository = ReportRepository::new(pool.clone());
    let reports = report_repository.get_by_game_id(game_id.try_into()?).await?;
    reports.iter().try_for_each(|report| report.validate())?;
    Ok(reports)
}
...
pub async fn update_report(pool: &SqlitePool, report_id: u64, report: Report) -> Result<bool, anyhow::Error> {
    let report_repository = ReportRepository::new(pool.clone());
    report.validate()?;
